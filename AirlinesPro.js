...
const showOnScreen = () => {
  let showFlightsOnScreen = confirm(
    "¿Desea ver en pantalla los vuelos programados?"
  );
...
const showFlights = () => {
  flights.forEach((flight) => {
    if (flight.layover === true) {
      alert(
        `El vuelo con origen: ${flight.from} y con destino: ${flight.to}. Tiene un precio de: ${flight.cost}€ y efectúa escalas.`
      );
    } else {
      alert(
        `El vuelo con origen: ${flight.from} y con destino ${flight.to}. Tiene un precio de ${flight.cost}€ y no realiza escalas.`
      );
    }
  });
};
...
const addFlights = () => {
  ...
  if (addNewFlight === true) {
    if (flights.length >= 15) {
      console.table(flights);
      alert(
        "Para introducir un nuevo vuelo, primero tienes que eliminar alguno ya almacenado"
      );
      deleteFlights();
    }
    ...
    addFlight.layover = confirm(
      "¿El vuelo realiza alguna escala?(Aceptar para confirmar)"
    );
...
const deleteFlights = () => {
  ...
  if (deleteFlight === true) {
    let deleteFLightById = +prompt(
      "Por favor introduzca el id del vuelo que va eliminar"
    );
    for (let i = 0; i < flights.length; i++) {
...
const searchByPrice = () => {
  ...
  if (Number.isNaN(getPrice)) {
    ...
    if (potentialFlights.length === 0) {
      ...
      alert(
        "Los vuelos que podrían interesarle son los mostrados en la tabla inferior:⬇️"
      );
